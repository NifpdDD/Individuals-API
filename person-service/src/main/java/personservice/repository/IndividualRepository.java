...
public interface IndividualRepository extends JpaRepository<Individual, UUID> {
    ...
    @Query(
            value = "FROM Individual i WHERE (:emails) IS NULL OR i.user.email IN :emails",
            countQuery = "SELECT COUNT(i) FROM Individual i WHERE (:emails) IS NULL OR i.user.email IN :emails"
    )
    ...
    @Query("""
            ...
            """)
    void sotfDelete(@Param("id") UUID id);
}
